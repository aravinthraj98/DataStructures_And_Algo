...
//It is String.length-LPS
//mindeletion=mininsertion 
public class MinimumDeletionInPalindrome {
...
//					System.out.print(i+""+j);
	...
	public static void main(String[] args) {
		...
		System.out.print("BruteForce-->"+(n.length()-getWays(n.length(),m.length(),n,m)));
		int dp[][]=new int[n.length()+1][m.length()+1];
		for(int temp[]:dp) {
			Arrays.fill(temp, -1);
		}
		System.out.println("Memoize-->"+(n.length()-getWaysMemoize(n.length(),m.length(),n,m,dp)));
		System.out.println("tabulation-->"+(n.length()-getWaysTab(n.length(),m.length(),n,m,new int[n.length()+1][m.length()+1])));
