...
//Repeating sequence is LCS i!=j
public class LongestRepeatingSequence {

	static int getWays(int n,int m,String m1 ,String m2) {
		if(n==0 || m==0) return 0;
		if(m1.charAt(n-1)==m2.charAt(m-1) && n!=m)return 1+getWays(n-1,m-1,m1,m2);
	...
	static int getWaysMemoize(int n,int m,String m1,String m2,int[][] dp) {
		...
		if(n==0 || m==0) return 0;
		if(m1.charAt(n-1)==m2.charAt(m-1) && n!=m) {
...
//				System.out.print(m1.charAt(i-1)+""+m2.charAt(j-1));
				if(m1.charAt(i-1)==m2.charAt(j-1) && i!=j) {
					dp[i][j]=1+dp[i-1][j-1];
